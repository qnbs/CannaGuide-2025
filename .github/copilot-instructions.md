...
- **Testing:** Vitest (2688 tests) + Playwright E2E + Playwright Component Tests
...
7. **Local AI Stack (ADR-0011):** 29 service modules in `services/local-ai/` orchestrate on-device ML, organized in 9 layers:
    - **core/** -- `localAI.ts` (facade, BaseAIProvider), `LocalAIInfrastructure.ts` (singleton), `inferenceRouter.ts` (3-layer routing), `infrastructureService.ts` (barrel)
    - **models/** -- `modelManager.ts` (pipeline lifecycle), `modelLoader.ts` (ONNX WebGPU/WASM), `webLlmService.ts` (WebLLM lifecycle), `webLlmModelCatalog.ts` (4-model catalog), `preloadOrchestrator.ts` (8-step preload), `webLlmDiagnosticsService.ts`
    - **inference/** -- `inferenceQueue.ts` (priority queue), `streamingService.ts` (SSE-style), `promptHandlers.ts` (all AI features)
    - **vision/** -- `diagnosisService.ts` (ONNX leaf classification), `imageSimilarityService.ts` (CLIP), `plantDiseaseModelService.ts` (PlantVillage)
    - **nlp/** -- `nlpService.ts` (sentiment/summarization/zero-shot), `embeddingService.ts` (MiniLM-L6), `languageDetection.ts` (EN/DE), `ragEmbeddingCacheService.ts` (IndexedDB LRU), `whisperService.ts` (V-06 offline STT via Whisper-Tiny)
    - **device/** -- `gpuResourceManager.ts` (GPU mutex), `webGpuService.ts` (adapter lifecycle), `healthService.ts` (device classification), `ecoModeService.ts`, `preloadService.ts`
    - **cache/** -- `cacheService.ts` (IndexedDB 256 entries, 7d TTL), `progressEmitter.ts` (WebLLM download progress)
    - **telemetry/** -- `telemetryService.ts` (latency/success tracking, fallback breakdown)
    - **fallback/** -- `fallbackService.ts` (~1600 LOC heuristic fallback)
    - Public API: `interfaces.ts` (20+ contracts), `index.ts` (barrel). All backward-compat stubs removed (Phase 3 complete).

8. **Worker Bus:** `workerBus.ts` provides promise-based, type-safe worker communication with backpressure, retry, telemetry, AbortController support, Transferable zero-copy transfers, heap-based priority queue, and pagehide teardown. All 11 workers (VPD simulation, genealogy, scenario, inference, image generation, hydro forecast, terpene, vision inference, calculation, voice, simulation) use this bus. Priority levels: `critical` (VPD safety), `high` (user-initiated simulation), `normal` (default), `low` (ML inference, image gen). `PriorityQueue<T>` min-heap in `utils/priorityQueue.ts` with O(log n) enqueue/dequeue and FIFO tiebreaking. `getQueueState()` returns per-priority breakdown + `effectiveConcurrency`. W-01.1 Dynamic Concurrency -- `deviceCapabilities.ts` auto-scales concurrency per hardware (`hardwareConcurrency * 0.6`, clamped [2, 12], battery-aware). W-02 Priority Preemption -- when all slots are full and a higher-priority job arrives, the lowest-priority running job is preempted (AbortController-based, main-thread only, max 3 re-queues) and the higher-priority job takes the slot. W-02.1 Cooperative Preemption -- `workerAbort.ts` intercepts `__CANCEL__` messages in workers; `checkAborted(messageId)` throws in loops for early termination; `cooperativePreemptions` tracked in telemetry. W-03 COEP SharedArrayBuffer -- COEP `credentialless` enables SAB on Netlify/Vercel/Cloudflare; `crossOriginIsolation.ts` detects; `sharedBufferPool.ts` manages acquire/release with ArrayBuffer fallback (ADR-0009). W-04 Cross-Worker Channels -- `createChannel(workerA, workerB)` creates a MessageChannel and transfers ports to both workers via `__PORT_TRANSFER__` message; `closeChannel()` tears down; auto-cleanup on unregister/dispose. W-04.1 AtomicsChannel -- `atomicsChannel.ts` provides lock-free bidirectional signaling via SAB + Int32Array + Atomics (8 slots). W-04 Generic Typed Dispatch -- `WorkerMessageMap` in `workerBus.types.ts` maps worker names to per-message payload/response types; `dispatch()` overloads enforce compile-time type safety for typed workers (simulation, visionInference, hydroForecast); untyped workers fall through to `unknown`. W-05 Lock-Free Ring Buffer -- `lockFreeRingBuffer.ts` provides SPSC ring buffer on SAB for high-frequency data streaming (power-of-2 capacity, bitmask arithmetic, batch push/pop, blocking `waitForData()`). W-06 Worker Pool -- `workerPool.ts` provides centralized lifecycle management: lazy spawning via `getOrCreate()`, 45s idle timeout, hot-worker exemption (VPD, voice), device-aware pool sizing via `getMaxPoolSize()`. Factory registry `workerFactories.ts` with 10 entries. SAB hot-path auto-init on spawn (AtomicsChannel + LockFreeRingBuffer). Pool metrics flushed to Redux DevTools + Sentry (ADR-0010). `workerStateSyncService.ts` provides a framework-agnostic handler registry for automatic Redux/Zustand wiring from dispatch results. `workerTelemetryService.ts` connects to Sentry (10% error-rate alerts) and Redux DevTools (5s debounced `workerMetricsSlice`). See `docs/worker-bus.md`.
...
11. **Notification Service:** `nativeBridgeService.ts` provides unified notification dispatch with platform-aware routing. Tauri Desktop: delegates to `@tauri-apps/plugin-notification`. Web/PWA: uses the browser Notification API. Permissions requested at app bootstrap. Gracefully degrades to no-op when permissions are denied.
...
- Baseline: 2688 tests, 0 failures
...
- **Mutation Testing:** Stryker Mutator (`stryker.conf.json`) targets Redux slices, IoT store, calculator services, SAB utils, workerPool. `coverageAnalysis: "perTest"` + TypeScript checker for fast targeted runs. `concurrency: 6`, `timeoutMS: 120000`. Incremental cache persisted in CI via `actions/cache`. Run: `pnpm run test:mutate`. Break threshold: 50% mutation score. Reports in `reports/mutation/`.
...
| `apps/web/services/local-ai/core/localAI.ts`                                    | Pure facade implementing BaseAIProvider (delegates to router, manager, orchestrator)                                                                                         |
| `apps/web/services/local-ai/core/inferenceRouter.ts`                            | Cache -> WebLLM -> Transformers.js inference routing with retry + backoff                                                                                                    |
| `apps/web/services/local-ai/core/infrastructureService.ts`                      | Barrel re-export for LocalAIInfrastructure singleton                                                                                                                         |
| `apps/web/services/local-ai/models/modelManager.ts`                             | Pipeline lifecycle (text + vision), primary/alt model fallback, dispose                                                                                                      |
| `apps/web/services/local-ai/models/modelLoader.ts`                              | ONNX pipeline loader (WebGPU/WASM, concurrency guard)                                                                                                                        |
| `apps/web/services/local-ai/models/webLlmService.ts`                            | WebLLM lifecycle, model loading, progress tracking                                                                                                                           |
| `apps/web/services/local-ai/models/webLlmModelCatalog.ts`                       | WebLLM adapter layer: derives WEB_LLM_MODELS from @cannaguide/ai-core Model Registry, autoSelectModel() delegates to registry                                                |
| `apps/web/services/local-ai/models/preloadOrchestrator.ts`                      | 8-step preload sequence with progress callbacks and error counting                                                                                                           |
| `apps/web/services/local-ai/inference/inferenceQueue.ts`                        | Priority queue for inference tasks                                                                                                                                           |
| `apps/web/services/local-ai/inference/streamingService.ts`                      | SSE-style streaming for local text generation                                                                                                                                |
| `apps/web/services/local-ai/inference/promptHandlers.ts`                        | Prompt formatting for all AI features                                                                                                                                        |
| `apps/web/services/local-ai/vision/diagnosisService.ts`                         | Plant health diagnosis pipeline (ONNX classifyLeafImage, classifySeverity, enrichWithKnowledge)                                                                              |
| `apps/web/services/local-ai/vision/imageSimilarityService.ts`                   | CLIP image comparison, growth tracking                                                                                                                                       |
| `apps/web/services/local-ai/vision/plantDiseaseModelService.ts`                 | PlantVillage MobileNetV2 ONNX IndexedDB caching, download with progress, HEAD check                                                                                          |
| `apps/web/services/local-ai/nlp/nlpService.ts`                                  | NLP pipelines (sentiment, summarization, zero-shot)                                                                                                                          |
| `apps/web/services/local-ai/nlp/embeddingService.ts`                            | MiniLM embeddings, semantic ranking                                                                                                                                          |
| `apps/web/services/local-ai/nlp/languageDetection.ts`                           | On-device EN/DE language detection                                                                                                                                           |
| `apps/web/services/local-ai/nlp/ragEmbeddingCacheService.ts`                    | Persistent IndexedDB LRU embedding cache (model versioning, telemetry, background precompute)                                                                                |
| `apps/web/services/local-ai/nlp/whisperService.ts`                              | V-06 Whisper-Tiny offline STT: lazy pipeline loading, GPU mutex, transcribe(Float32Array) -> text                                                                            |
| `apps/web/services/local-ai/device/gpuResourceManager.ts`                       | GPU mutex v2: string registry, GpuPriority queue, auto-release 30s                                                                                                           |
| `apps/web/services/local-ai/device/webGpuService.ts`                            | Centralized WebGPU adapter, shared device lifecycle, feature detection                                                                                                       |
| `apps/web/services/local-ai/device/healthService.ts`                            | Device classification, health monitoring                                                                                                                                     |
...
| `apps/web/workers/visionInferenceWorker.ts`                                     | Off-main-thread ONNX inference: 38-class PlantVillage labels, `CANNABIS_MAP`, `autoWhiteBalance()` (Gray World), `preprocessImage()` (ImageNet CHW), WorkerBus               |
...
| `apps/web/services/workerBus.ts`                                                | Promise-based worker communication bus (11 workers), AbortController, Transferable, onDispatchComplete hook                                                                  |
| `apps/web/services/workerPool.ts`                                               | Centralized Worker lifecycle: lazy spawning via `getOrCreate()`, 45s idle timeout, hot-worker exemption (VPD, voice), device-aware pool sizing, SAB hot-path init (ADR-0010) |
| `apps/web/services/workerFactories.ts`                                          | Centralized factory registry for 10 workers (name -> factory + hot flag + priority)                                                                                          |
...
| `apps/web/services/voiceOrchestratorService.ts`                                 | Central voice state machine (IDLE/LISTENING/PROCESSING/SPEAKING/CONFIRMATION), command matching, confirmation loops, error recovery                                          |
| `apps/web/services/porcupineWakeWordService.ts`                                 | Porcupine WASM wake-word detection: BYOK AccessKey, 11 built-in keywords, on-device only                                                                                     |
| `apps/web/services/cloudTtsService.ts`                                          | ElevenLabs Cloud TTS: BYOK AES-256-GCM encrypted key, rate-limited, AudioContext playback                                                                                    |
| `apps/web/services/speakNaturalService.ts`                                      | TTS text normalization: 30+ abbreviation expansions, markdown strip, German decimals                                                                                         |
...
| `apps/web/services/nativeBridgeService.ts`                                      | Unified notification dispatch (Tauri plugin-notification / Web Notification API) with platform-aware routing                                                                 |
| `apps/web/services/tauriDialogService.ts`                                       | Native file dialogs for Tauri Desktop (save/open via plugin-dialog + IPC export_data/import_data)                                                                            |
...
| `apps/web/utils/deviceCapabilities.ts`                                          | Adaptive concurrency: `getDeviceConcurrencyLimit()` (sync), `getAdaptiveConcurrencyLimit()` (async, battery-aware)                                                           |
| `apps/web/utils/workerAbort.ts`                                                 | Worker-side cooperative preemption: `initAbortHandler()`, `checkAborted()`, `clearAborted()`, `isAborted()`                                                                  |
| `apps/web/utils/workerSabHandler.ts`                                            | Worker-side SAB init: `initSabHandler()` intercepts `__ATOMICS_CHANNEL__`/`__RING_BUFFER__` messages, `getWorkerChannel()`, `getWorkerRingBuffer()`                          |
| `apps/web/utils/crossOriginIsolation.ts`                                        | SAB feature detection: `isCrossOriginIsolated()`, `canUseSharedArrayBuffer()`, `getCrossOriginIsolationStatus()`                                                             |
| `apps/web/utils/sharedBufferPool.ts`                                            | SAB/ArrayBuffer pool: acquire/release with progressive enhancement fallback                                                                                                  |
| `apps/web/utils/atomicsChannel.ts`                                              | Lock-free bidirectional signaling via SAB + Int32Array + Atomics (8 slots)                                                                                                   |
| `apps/web/utils/lockFreeRingBuffer.ts`                                          | SPSC ring buffer on SAB: power-of-2 capacity, bitmask arithmetic, batch push/pop, blocking waitForData                                                                       |
...
| `apps/web/utils/opfsStorage.ts`                                                 | OPFS model persistence: write/read/delete/list/clear, scoped 'cannaguide-ml-cache' directory, isOpfsAvailable() feature detection                                            |
...
| `packages/ai-core/src/modelRegistry.ts`                                         | Canonical local AI model registry (4 models, GPU-tier selection, version tracking, MODEL_REGISTRY_VERSION)                                                                   |
...
| `apps/desktop/package.json`                                                     | @cannaguide/desktop -- Tauri v2 desktop app package (9 plugins)                                                                                                              |
| `apps/desktop/src-tauri/tauri.conf.json`                                        | Tauri v2 config: window, CSP, bundle, updater, frontendDist -> ../web/dist                                                                                                   |
| `apps/desktop/src-tauri/Cargo.toml`                                             | Rust dependencies: tauri 2, 9 plugins, serde, chrono                                                                                                                         |
| `apps/desktop/src-tauri/capabilities/*.json`                                    | 10 modular capability files: core, desktop, fs, dialog, notification, tray, shortcut, updater, window-state, store (ADR-0012)                                                |
| `apps/desktop/src-tauri/src/lib.rs`                                             | Tauri IPC commands: get_app_version, export_data, import_data; 9 plugin init                                                                                                 |
| `docs/adr/0012-desktop-tauri-architecture.md`                                   | ADR: Platform detection, capability structure, IPC boundaries, plugin config                                                                                                 |
