...
7. **Local AI Stack (ADR-0011):** 29 service modules in `services/local-ai/` orchestrate on-device ML, organized in 9 layers:
    ...
    - **nlp/** -- `nlpService.ts` (sentiment/summarization/zero-shot), `embeddingService.ts` (MiniLM-L6), `languageDetection.ts` (EN/DE), `ragEmbeddingCacheService.ts` (IndexedDB LRU)
    ...
    - Public API: `interfaces.ts` (20+ contracts), `index.ts` (barrel). 29 backward-compat stubs at old paths (Phase 3 removal).
