...
- **feat(ui):** Categorical chart palette — `--color-chart-1..8` (theme-independent, CVD-safe: validated via the data-viz method against the darkest theme chart surface, worst adjacent CVD separation 8.4 / normal-vision 19.3) plus `--color-chart-grid/-axis/-label` chrome. Exposed as `chart-*` / `muted` Tailwind colors in the preset and mirrored for Recharts `fill`/`stroke` props (which don't resolve CSS `var()`) in `apps/web/utils/chartPalette.ts` — one authoritative palette to replace the ad-hoc per-chart hex. Assigned in fixed slot order (a 9th series folds to "Other"). Chart-surface migration onto it follows (WS-A)
- **feat(ui):** Typography micro-scale tokens — `--font-size-2xs`/`3xs` (11px/10px) + line-heights in `packages/ui/src/tokens.css`, exposed as the `text-2xs`/`text-3xs` utilities via the Tailwind preset, so data-dense labels stop bypassing the token system with ad-hoc `text-[10px]`/`text-[11px]`/`text-[0.68rem]` arbitrary values (WS-A). Standard sizes keep Tailwind's defaults — only the missing micro tier is tokenised
- **ci(a11y):** `check-contrast.mjs` WCAG-AA theme-contrast audit — reconstructs all 9 themes from `tokens.css` (midnight base + per-theme overrides, matching the CSS cascade) and checks curated text/interactive foreground↔background pairs against AA (4.5 / 3.0 large). Advisory step in the `verify` CI job (Lint, Types + Gates): AA findings are non-blocking, but the audit **fails closed** (non-zero exit) if a theme or a curated token goes missing, so a rename can't silently shrink coverage. `--strict` makes AA findings blocking too; `--json` for machine output. Zero-dependency, run via `node scripts/check-contrast.mjs` (kept out of root `package.json` scripts so a push never widens the scoped typecheck). Baseline: 14 pairs below AA across the 9 themes — the WS-A2 fix list
...
- **ci(security):** `check-override-floors.mjs` override/dependabot drift gate — the `overrides:` block in `pnpm-workspace.yaml` and the `ignore:` list in `.github/dependabot.yml` are a load-bearing pair (a package is silenced there **because** it is pinned here), and nothing re-validated them. The gate fails when the `overrides:` block disagrees with the copy pnpm mirrors into `pnpm-lock.yaml` (`--frozen-lockfile` refuses to install on any difference, so an override edited without regenerating the lockfile turns every CI job red in its install step), when a fully-ignored npm package has no override, when a major-scoped key (`js-yaml@3`) matches no version resolved in `pnpm-lock.yaml` — an orphaned pin that protects nothing while Dependabot stays muted — and when a **new** override floor is added without an upper bound. The 12 pre-existing unbounded pins sit in an explicit `LEGACY_UNBOUNDED` allowlist that **may only shrink** (bounding one without delisting it also fails), because retiring them is not mechanical: three have already crossed a major — `uuid >=11.1.1` resolved to **14.0.0** via zustand, `basic-ftp >=5.3.1` to **6.0.1** via get-uri, `linkify-it >=5.0.1` to **6.0.0** via markdown-it — and `@babel/core >=7.29.6` resolves to nothing at all, since neither `@babel/core` nor `@stryker-mutator` is in the lockfile and the "devDep path via Stryker" it cites no longer exists. Zero-dependency, offline (lockfile + config only), wired into the `Security` CI job
...
- **perf(pwa):** The service worker no longer precaches the local-AI runtime. `injectManifest.globIgnores` now excludes `ai-runtime` (WebLLM + tvmjs), `transformers`, `ort.bundle.min`, the ORT WASM binaries and `three`. Those chunks are lazily `import()`ed from `packages/ai-core/src/ml.ts` and only reachable if the user turns local AI on — but the precache downloaded them at service-worker install for **every** visitor on **every** device, defeating the lazy boundary entirely. Measured against the generated manifest: **247 entries / 19.15 MiB -> 243 entries / 10.57 MiB (-45%)**. The two caching paths are separate and were previously conflated here: remote ML model artifacts stay with `handleMlModelRequest` (LRU-capped `cannaguide-ml-models` cache, gated on `ML_MODEL_HOSTS`), while the ignored chunks are **same-origin** Vite output and never reach that route -- they are cached by the hashed-asset rule after their first fetch. So they move from "downloaded up front" to "cached once actually used", with one honest caveat: if an update lands and the new runtime chunk has not been fetched yet, local AI cannot be started for the first time offline
- **feat(settings):** Automatic offline-model download is now **opt-in** (`localAi.autoPreloadOnStartup`, default `false`). `scheduleIdlePreload()` ran unconditionally on every boot and fetched the `standard` tier — nine ONNX models, hundreds of MB from the HuggingFace CDN — with no setting, no consent and no metered-network check. The metered/Data-Saver guard `isNetworkSuitableForBulkDownload()` existed but was reachable **only** through the `full` tier, a branch the startup path never took, so `saveData` was ignored in practice. It now guards the automatic path directly. An explicit "Preload" from the Local AI settings card is deliberately unaffected — there the user has asked for it. The setting is exposed as a toggle in that same card (translated in all five locales); the guard also rejects `connection.type === 'cellular'` outright, since a 4G link reports `effectiveType: '4g'` and would otherwise look exactly like home broadband, and it is re-checked immediately before the download starts rather than only when the idle callback is scheduled
- **ci:** The bundle budget has teeth again. `ai-runtime` was exempt **by name** and had grown to 2.29 MB gzip (~2.4x its size at the previous audit) while the gate printed `[PASS]` throughout. The blanket exempt list is replaced with explicit per-chunk ceilings, and a **precache budget (12 MiB)** is added — nothing measured the precache before; the only constraint was workbox's per-file `maximumFileSizeToCacheInBytes`, an inclusion filter rather than a gate, with its size warning explicitly silenced. Verified against the pre-change build: the gate reports `Precache is 19.15 MiB, over the 12 MiB budget` with the fix in the message

### Security

- **fix(ci):** The required `CI Status` check no longer passes while Chromium E2E is still running. `e2e` was missing from the gate's `needs:` list, so the check aggregated only `build`, `test`, `verify`, `security` and `rust` -- and reached SUCCESS with both browser shards `in_progress`. This was observed live three separate times on PR #474: the PR was formally mergeable, by the repository's own required check, with the browser tests unfinished, and a shard going red afterwards changed nothing. `e2e` is now in `needs:` and asserted in the aggregate step against `== "success"` specifically, so `cancelled`, `skipped` and an empty result all fail rather than only an explicit `failure`. `e2e-cross-browser` (Firefox/WebKit) is unchanged and remains advisory
- **fix(pwa):** onnxruntime-web's 25.6 MiB wasm is no longer shipped. `ort-wasm-simd-threaded.jsep.wasm` was bundled into every deployment, which exceeded **Cloudflare Pages' hard 25 MiB per-file limit** -- so that mirror could never deploy at all, a failure hidden behind `continue-on-error` until #488 removed it. Both loaders in `@cannaguide/ai-core/ml` now point ORT at `cdn.jsdelivr.net` (already in the CSP `connect-src`), and a post-build step drops the now-unused binary from `dist`. Fixes a second, independent defect found on the way: two workers hard-coded `onnxruntime-web@1.20.0` in their CDN URL while the package resolves to **1.27.0** -- ORT's JS and wasm are built together and are not interchangeable, so those inference paths were loading a mismatched binary. The version now lives in one constant (`ORT_VERSION`), is set at the loader seam so no call site can forget it, and `ortVersionPin.test.ts` asserts it against the installed package so the pin cannot drift silently again
- **fix(ci):** Cloudflare Pages reported success for deploys that never happened. The workflow carried `continue-on-error: true` at **both** job and step level, and pinned the deploy action to `wranglerVersion: '4.105.0'` while `package.json` carries `^4.110.0` -- so the action tried to install a different version at the repo root and aborted with `ERR_PNPM_ADDING_TO_ROOT`. Observed on PR #474: the check was green, the deploy had failed, and the preview-URL comment was silently skipped. Both `continue-on-error` flags are removed, so a failed deploy is now red. The workflow stays **advisory by not being a required check** -- which is the honest mechanism; a red run still cannot block a merge. The deploy itself now runs the lockfile-pinned CLI via `pnpm exec wrangler` instead of the action, so there is no second source of truth for the wrangler version and nothing to install at deploy time; it also fails when wrangler exits 0 without printing a deployment URL. A pre-deploy step now rejects any artifact over **Cloudflare Pages' 25 MiB per-file limit** before upload, naming the offending files, instead of letting wrangler fail late. A post-deploy check reads back the served `/version.json` and **fails closed**: it polls up to 15 times to absorb edge-cache lag, then errors with the last served commit if the expected one never appears
- **fix(ci):** Build artifacts could carry another commit's identity. `dist/version.json` was written by a Vite plugin **inside** the turbo `build` / `build:gh` tasks, whose cached output is `dist/**` -- so on a cache hit the plugin never ran and the artifact replayed the commit that first populated the cache. Reproduced live: a Vercel preview built for `95848cf` served `{"commit":"3501ef2"}`. The task `env` lists were `["BUILD_BASE_PATH", "VITE_*"]`, so neither `GITHUB_SHA` nor `VERCEL_GIT_COMMIT_SHA` affected the hash, and `apps/web/package.json` was absent from the task inputs, so a version bump could not bust the cache either. Both build tasks were affected, so GitHub Pages had the same defect as Vercel -- passing `GITHUB_SHA` into the Pages build did not help, because a cache hit skips the build entirely. Commit identity is now stamped by `scripts/stamp-build-metadata.mjs` in an **uncached** step after the build, fed the right SHA per platform (`DEPLOY_SHA` on Pages, `VERCEL_GIT_COMMIT_SHA` on Vercel, the tag's resolved commit on release, the PR head on CI), and asserted with `--verify` **before** upload/deploy rather than only after -- a post-deploy check fails after the bad artifact is already serving traffic, and a red workflow does not roll Pages back. Full 40-character SHAs are now required and short ones rejected: the old fallback used `rev-parse --short`, so Vercel stamped 7 characters while every consumer compared against 40. `apps/web/package.json` added to both tasks' inputs
- **fix(ci):** Release integrity — four ways a release could misrepresent what it contains:
    - **`release-publish.yml` never compared the tag to the tree it labels.** No semver validation, no cross-check. Pushing `v9.9.9` on a 1.10.0 checkout produced a GitHub Release named "CannaGuide v9.9.9" containing a 1.10.0 build, with a CycloneDX SBOM and a build-provenance attestation vouching for it. The tag is now asserted against both `package.json` and `apps/web/package.json` (which is what actually ships and stamps `dist/version.json`) on the tag's own commit after it is checked out, with the semver **shape** check running before `Ensure tag exists` so a malformed input never becomes a real tag
    - **`release-gate.yml` could not block `release-publish.yml`.** Both fire on the same `push: tags: ['v*']` event and ran concurrently with no dependency, so a tag whose gate **failed** still published a release. Actions has no cross-workflow `needs:`, so publish now polls the Actions API for the Release Gate run on the same commit and fails closed on failure, on timeout, and on the gate never having run at all
    - **The CI-Status guard warned and continued** when no `CI Status` check was found, and was skipped entirely for `workflow_dispatch` ("the maintainer is in control" — which was precisely the path with no verification). Combined with the docs-only short-circuit in `ci.yml`, a commit that never ran security checks could be released. Both holes are closed: the guard fails closed and now covers manual dispatch
    - **`deploy.yml` re-ran the production audit at `--audit-level=critical`**, the same inversion just fixed in `ci.yml`. Now `high`
- **ci(deploy):** Post-deploy version verification. The build has stamped `dist/version.json` with `{version, commit, builtAt}` for months, but **no workflow ever read it back** — the one consumer named in the code comment (`preview-validation.yml`) is disabled and contains no such query. GitHub Pages deploys now poll the live `version.json` and assert its `commit` equals the SHA that was built (an explicit `DEPLOY_SHA` derived from `workflow_run.head_sha`, which is also what the build stamps -- `github.sha` is the default-branch head under `workflow_run`, not the triggering commit), failing closed if the live site never reports it. The stale code comment is corrected
- **ci:** `check-audit-backlog.mjs` now blocks on **CRITICAL**. It bucketed `critical` into `counts.low` and gated only on `counts.high`, so an open critical audit item passed the release gate — the one severity the gate most obviously exists to stop
- **ci:** `timeout-minutes` on all five `deploy.yml` jobs (build 30, deploy 15, e2e-pages 30, lighthouse 30, cleanup 10). The production deploy path previously ran on the 6-hour default throughout
- **fix(desktop):** Remove the Tauri `export_data` / `import_data` commands entirely. Both took an absolute path straight from the webview and handed it to `std::fs::write` / `read_to_string`. Custom `#[tauri::command]` functions are **not** covered by the allow/deny lists in `capabilities/fs.json` -- those only constrain the `fs:` plugin -- so any script executing in the webview could call `invoke('export_data', { path: '/home/u/.ssh/authorized_keys', data })` or `invoke('import_data', { path: '/etc/shadow' })`, and the desktop CSP allows `script-src 'unsafe-inline'`. They were first hardened with a scoped-path check (`src/path_scope.rs`, permitted roots + extension allowlist + traversal/hidden-component rejection) and then with point-of-use symlink defences. Review found successive defects in that hardening -- a relative path was made absolute before validation, making the `NotAbsolute` rule unreachable from the real command path; and the symlink guard was a check-then-use race that survived two attempted fixes, because ordering alone still identifies the file by pathname on both sides. Each fix was correct and each was followed by another finding. The commands had **no production caller** at any point -- `tauriDialogService.ts` was imported only by its own test -- so the whole chain was dead code carrying live attack surface. Deleting it removes the vulnerability class rather than narrowing it: gone are the two commands, `path_scope.rs`, `tauriDialogService.ts` and its test. Should desktop file export be wanted later, the right shape is to open the dialog in Rust and act on the chosen handle there, never to accept a free-form path from the webview.
- **ci:** Rust is now type-checked on pull requests. `cargo fmt --check`, `clippy -D warnings` and `cargo test --lib` run in a new path-gated `Rust (Tauri)` job in `ci.yml` whenever `apps/desktop/**` changes, wired into the `CI Status` gate (required only when it actually ran, so a PR that does not touch the desktop is unaffected). Previously Rust was compiled **only** by `desktop-build.yml` on `v*` tags, so a broken Rust change was first discovered while cutting a release. The committed Rust was also not `rustfmt`-clean, which the new gate required normalising
...
- **ci(security):** Resolve the code-scanning **"configuration error"**. The advanced `codeql.yml` workflow had been `disabled_manually`, so CodeQL was running only via GitHub **Default Setup** — and a repo with _both_ an advanced config file and Default Setup is the dual source GitHub rejects. Re-enabled `codeql.yml` and switched Default Setup off, so advanced CodeQL is again the single source, matching the custom `codeql-config.yml` (paths-ignore + `security-and-quality`, 4 languages incl. rust). (The CHANGELOG had claimed this state; reality had drifted.)
- **ci(security):** Add top-level `permissions: contents: read` to `claude.yml` and `claude-code-review.yml` (they declared only job-level permissions), clearing the two Scorecard `TokenPermissions` **"no topLevel permission defined"** alerts. The other 12 Scorecard alerts are job-scoped `write` grants that are **required** (committing strain/graph data, deploying, deleting stale branches, publishing releases, uploading SARIF) — job-level scoping is the recommended least-privilege pattern (Scorecard 9/10), dismissed as won't-fix with justification
...
- **chore(deps):** Refresh the decayed override floors in `pnpm-workspace.yaml`, clearing all 22 advisories (14 high / 7 moderate / 1 low). **Every floor touched here is bounded (`>=x.y.z <next-major`)**; the 12 pre-existing unbounded pins are ratcheted rather than changed (see the `check-override-floors.mjs` entry above). An open-ended floor resolves to the newest major in the registry, and that is what actually went wrong here:
