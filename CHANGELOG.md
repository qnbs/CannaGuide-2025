...
- **fix(desktop):** Scope the Tauri `export_data` / `import_data` commands. Both took an absolute path straight from the webview and handed it to `std::fs::write` / `read_to_string`. Custom `#[tauri::command]` functions are **not** covered by the allow/deny lists in `capabilities/fs.json` — those only constrain the `fs:` plugin — so any script executing in the webview could call `invoke('export_data', { path: '/home/u/.ssh/authorized_keys', data })` or `invoke('import_data', { path: '/etc/shadow' })`. The frontend picks the path with a native dialog, but nothing bound the invoked path to that dialog result, and the desktop CSP allows `script-src 'unsafe-inline'`, so any HTML-injection bug became arbitrary local file read/write. Paths are now validated against the app data dir plus Documents/Downloads and restricted to `.json`/`.cannaguide`; `..` traversal, hidden components _below the matched root_, and symlinked targets are refused (paths such as `.ssh`/`.config`/`.gnupg` are outside every permitted root and rejected by the root check itself; the hidden rule cannot apply above the root because the Linux app data dir is `~/.local/share/cannaguide`), and the parent is canonicalised so a symlinked directory cannot smuggle the write out of scope. The check is a pure, filesystem-free function in `src/path_scope.rs` with 14 unit tests, plus 9 filesystem-backed tests in `lib.rs` covering the command entry point itself. Two defects found in review are fixed with it: a relative path was made absolute (against the process working directory) by `resolve_candidate` **before** validation, so the `NotAbsolute` rule was unreachable from the real command path and only the pure test passed; and the symlink defence was a check-then-use race, since `fs::write` / `read_to_string` follow symlinks and nothing bound the earlier `symlink_metadata` observation to the later open. Writes now go to a fresh `create_new` temp file in the canonicalised parent and are `rename`d over the target (which _replaces_ a symlink instead of writing through it, and makes the export atomic); reads bind the check to the open by file **identity** — stat the path, open it, then require the handle to be that same `dev`+`ino`, so a traversed symlink (a different inode from its target) or a file swapped in between the two calls is refused, and every read goes through the handle rather than the path. Ordering alone is not enough in either direction: checking before the open is the classic race, and checking after is also racy, because an attacker who removes the symlink once `File::open` has followed it leaves an ordinary file behind for the check to find. On Windows the inode comparison has no stable std equivalent (`windows_by_handle`), so that path stays check-then-use and says so rather than claiming a guarantee it does not provide. The pre-check remains only as a fast, advisory rejection. Note the commands currently have **no production caller** — `tauriDialogService.ts` is imported only by its own test — so this was latent attack surface rather than an exercised path; deleting the pair entirely remains an option
