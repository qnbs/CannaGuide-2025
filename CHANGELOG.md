...
- **ci(a11y):** `check-contrast.mjs` WCAG-AA theme-contrast audit — reconstructs all 9 themes from `tokens.css` (midnight base + per-theme overrides, matching the CSS cascade) and checks curated text/interactive foreground↔background pairs against AA (4.5 / 3.0 large). Advisory step in the `quality` CI job; `--strict` for a blocking exit, `--json` for machine output. Zero-dependency, run via `node scripts/check-contrast.mjs` (kept out of root `package.json` scripts so a push never widens the scoped typecheck). Baseline: 13 pairs below AA across the 9 themes — the WS-A2 fix list
