...
- **perf(pwa):** The service worker no longer precaches the local-AI runtime. `injectManifest.globIgnores` now excludes `ai-runtime` (WebLLM + tvmjs), `transformers`, `ort.bundle.min`, the ORT WASM binaries and `three`. Those chunks are lazily `import()`ed from `packages/ai-core/src/ml.ts` and only reachable if the user turns local AI on — but the precache downloaded them at service-worker install for **every** visitor on **every** device, defeating the lazy boundary entirely. Measured against the generated manifest: **247 entries / 19.15 MiB -> 243 entries / 10.57 MiB (-45%)**. Offline availability is unchanged: `public/sw.js` already caches ML assets on demand via `handleMlModelRequest` (LRU-capped `cannaguide-ml-models` cache), so they move from "downloaded up front" to "cached once actually used"
