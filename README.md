...
Comprehensive 4-tab help system with 138 total help items:

- **User Manual** (7 sections) -- step-by-step app usage guides covering all major features including Hydro Monitor, Grow Tech, IoT Dashboard, Disease Atlas, Learning Paths, Calculator Hub, and Lexikon
- **Searchable Lexicon** (91 terms) -- grower glossary across 6 categories: General/Techniques, Cannabinoids (THC, CBD, CBG, CBN, CBC, THCV, CBDV), Terpenes (10), Flavonoids (7), Nutrients (16 macro/micro), Diseases/Problems (13)
...
All calls route through `aiFacade.ts` -> `aiService.ts` -> `aiProviderService.ts` -> provider-specific services. Rate-limited (15 req/min sliding window). Structured JSON output via `responseSchema` with Zod validation.
...
| `localAiWebGpuService.ts`            | Centralized WebGPU adapter, device lifecycle, feature detection |
...
- `growLogRagService`: Token-based journal search with dynamic top-K and sliding window retrieval (recency + semantic)
...
i18next with 12 namespaces, EN + DE + ES + FR + NL:
...
pnpm --filter @cannaguide/web dev       # Vite dev server
pnpm --filter @cannaguide/web build     # Production build
pnpm --filter @cannaguide/web test      # Vitest unit/integration
...
pnpm --filter @cannaguide/web typecheck # tsc --noEmit
```
...
packages/
  ...
  ui/                       Shared design tokens (9 themes) + Tailwind preset
...
Contributions welcome! See [CONTRIBUTING.md](CONTRIBUTING.md) for guidelines. API reference: [docs/api/](docs/api/).
...
| **v1.6** | ✅ Released | Forensic audit fixes, i18n completeness (5 languages), Intl.DateTimeFormat weekdays, metrics/diagnosis/planner slices, documentation sync, 1961 tests                                                                              |
...
| **3. Core Development**          | GitHub Codespaces + VS Code Copilot (Claude Opus 4.6) | Primary iteration engine -- feature refinement, security hardening, 1961 tests, CI/CD pipeline, local AI stack, documentation | v1.0 -> v1.6 |
...
**Infrastructure:** GitHub Pages, Netlify, TurboRepo
...
[![CodeQL](https://github.com/qnbs/CannaGuide-2025/actions/workflows/codeql.yml/badge.svg)](https://github.com/qnbs/CannaGuide-2025/actions/workflows/codeql.yml)
[![Deploy](https://github.com/qnbs/CannaGuide-2025/actions/workflows/deploy.yml/badge.svg)](https://github.com/qnbs/CannaGuide-2025/actions/workflows/deploy.yml)
[![Tests](https://img.shields.io/badge/Tests-1961%20bestanden-brightgreen)](https://github.com/qnbs/CannaGuide-2025/actions/workflows/ci.yml)
...
**Kennzahlen:** 776 Sorten -- 1961 Tests -- 109 Services -- 18 Redux Slices + 8 Zustand Stores -- 25 Custom Hooks -- 12 i18n-Namensraeume -- 9 Themes -- 21 CI-Workflows
...
| **Testing**        | Vitest 4.1, Playwright 1.58          | 1961 Unit-, E2E-, Komponenten-Tests           |
...
Umfassendes 4-Tab-Hilfesystem mit 138 Hilfe-Eintraegen insgesamt:

- **Benutzerhandbuch** (7 Sektionen) -- Schritt-fuer-Schritt-Anleitungen fuer alle wichtigen Funktionen inkl. Hydro Monitor, Grow Tech, IoT Dashboard, Krankheitsatlas, Lernpfade, Rechner-Hub und Lexikon
- **Durchsuchbares Lexikon** (91 Begriffe) -- Grower-Glossar in 6 Kategorien: Allgemein/Techniken, Cannabinoide (THC, CBD, CBG, CBN, CBC, THCV, CBDV), Terpene (10), Flavonoide (7), Naehrstoffe (16 Makro-/Mikro-), Krankheiten/Probleme (13)
...
| `LocalAIInfrastructure.ts`           | Einheitliche Cache + Telemetrie + Preload-Klasse                  |
| `localAiInfrastructureService.ts`    | Rueckwaertskompatibles Barrel-Export fuer LocalAIInfrastructure   |
| `localAiWebGpuService.ts`            | Zentraler WebGPU-Adapter, Geraete-Lebenszyklus, Feature-Erkennung |
...
- `growLogRagService`: Token-basierte Journal-Suche mit dynamischem Top-K und Sliding-Window-Retrieval (Aktualitaet + Semantik)
...
i18next mit 12 Namensraeumen, EN + DE + ES + FR + NL:
...
pnpm run test:run         # turbo run test:run (Einzellauf, 1961 Tests)
pnpm run lint             # turbo run lint
pnpm run typecheck        # turbo run typecheck
pnpm run format           # Prettier
pnpm run security:scan    # Vollstaendiger Sicherheits-Scan

# Web-App (Workspace-spezifisch)
pnpm --filter @cannaguide/web dev       # Vite Dev-Server
pnpm --filter @cannaguide/web build     # Produktions-Build
pnpm --filter @cannaguide/web test      # Vitest Unit/Integration
pnpm --filter @cannaguide/web test:e2e  # Playwright E2E (erfordert Build)
pnpm --filter @cannaguide/web test:ct   # Playwright Komponenten-Tests
pnpm --filter @cannaguide/web typecheck # tsc --noEmit
```
...
turbo.json                 TurboRepo-Pipeline (build, dev, test, lint, typecheck)
tsconfig.json              Nur Referenzen (apps/web, packages/*)
...
| Job                | Beschreibung                                                                 |
| ------------------ | ---------------------------------------------------------------------------- |
| Quality Gates      | Lint, Typecheck (Root + Workspaces via Turbo), 1961 Tests, Produktions-Build |
| 🛡 Security        | pnpm audit (critical), Trojan-Source-Scan, Gitleaks-Secret-Scan              |
...
Beiträge willkommen! Siehe [CONTRIBUTING.md](CONTRIBUTING.md) für Richtlinien. API-Referenz: [docs/api/](docs/api/).
...
| **v1.6** | ✅ Veröffentlicht | Forensic-Audit-Fixes, i18n-Vollstaendigkeit (5 Sprachen), Intl.DateTimeFormat-Wochentage, Metrics/Diagnosis/Planner-Slices, Dokumentations-Sync, 1961 Tests                                                                                                     |
...
| **3. Kernentwicklung**           | GitHub Codespaces + VS Code Copilot (Claude Opus 4.6) | Primaere Iterations-Engine -- Feature-Verfeinerung, Security-Hardening, 1961 Tests, CI/CD-Pipeline, lokaler KI-Stack, Dokumentation | v1.0 -> v1.6   |
...
**Infrastruktur:** GitHub Pages, Netlify, TurboRepo
