...
export const ActionToolbar: React.FC<ActionToolbarProps> = memo(({ onLogAction }) => {
    ...
    const actions: { type: ModalType; label: string; icon: React.ReactNode }[] = [
        ...
        },
        {
            ...
            icon: <PhosphorIcons.TestTube className="text-amber-400" />,
        },
        {
            type: 'training',
            label: t('plantsView.detailedView.journalFilters.training'),
            icon: <PhosphorIcons.Scissors className="text-purple-400" />,
        },
        {
            type: 'observation',
            label: t('plantsView.detailedView.journalFilters.observation'),
            icon: <PhosphorIcons.MagnifyingGlass className="text-slate-300" />,
        },
        {
            type: 'photo',
            label: t('plantsView.detailedView.journalFilters.photo'),
            icon: <PhosphorIcons.Camera className="text-cyan-400" />,
        },
        {
            type: 'pestControl',
            label: t('plantsView.detailedView.journalFilters.pestControl'),
            icon: <PhosphorIcons.WarningCircle className="text-red-400" />,
        },
        {
            type: 'amendment',
            label: t('plantsView.detailedView.journalFilters.amendment'),
            icon: <PhosphorIcons.Flask className="text-emerald-400" />,
        },
    ]
