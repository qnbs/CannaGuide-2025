...
const Leaf: React.FC<{
    ...
    rotation: number
    color: string
}> = ({ y, size, direction, rotation, color }) => {
    const d =
        direction === 'left'
...
const Bud: React.FC<{ cx: number; cy: number; size: number; color: string }> = ({
    cx,
    cy,
    size,
    color,
...
export const PlantVisualizer: React.FC<PlantVisualizerProps> = memo(({ plant, className }) => {
    ...
    const isDryingOrCuring = [PlantStage.Drying, PlantStage.Curing, PlantStage.Finished].includes(
        stage,
    )
    const leafColor = isDryingOrCuring
        ? `rgba(180, 160, 100, ${healthFactor})`
        : `rgba(var(--color-accent-500), ${mapRange(healthFactor, 0, 1, 0.4, 1)})`
    const stemColor = isDryingOrCuring
    ...
    const stemHeight =
        stage === PlantStage.Vegetative || stage === PlantStage.Flowering
            ? Math.max(8, stemBaseHeight * heightScale)
            : 4
                ...
                >
                    <g transform={`scale(${scale}) translate(0, ${yOffset})`}>
                        {stage === PlantStage.Seed ? (
                            ...
                            <>
                                <path
                                    d="M12 16 A 4 2 0 0 0 12 12 A 4 2 0 0 0 12 16 Z"
                                    fill="#A16207"
                                    opacity="0.6"
                                />
                                <path
...
PlantVisualizer.displayName = 'PlantVisualizer'
