...
export const VPDChart: React.FC<VPDChartProps> = React.memo(({ plant }) => {
    ...
    const input = useMemo(
        () => vpdService.createInputFromPlant(plant, simulationSettings),
                ...
                {latest && (
                    <div className="rounded-lg bg-slate-800/60 p-3 text-sm text-slate-200">
                        <p className="font-semibold text-slate-100">
                            {t('plantsView.vpd.currentVpd')}: {latest.vpd.toFixed(2)} kPa
                        </p>
                        <p className="text-slate-300">
                            {t('plantsView.vpd.targetBand', { band: t(band.labelKey) })}:{' '}
                            {band.min.toFixed(1)}-{band.max.toFixed(1)} kPa
                        </p>
                        <p className="text-slate-300">
                            {t('plantsView.vpd.status')}:{' '}
                            <span className="font-semibold uppercase">{latest.status}</span>
                        </p>
