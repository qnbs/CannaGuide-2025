import type { StrainTranslationData } from '@/types';

export const strains: Record<string, StrainTranslationData> = {
  "gorilla-butter": {
    "description": "Gorilla Butter ist eine potente, indica-dominante Hybridsorte, die für ihre starke Wirkung und ihr komplexes, erdiges Aroma bekannt ist. Sie ist der Elternteil des Phänotyps White Truffle. Die Wirkung ist stark entspannend und beruhigend, oft mit einem euphorischen Auftakt, der in eine tiefe körperliche Entspannung übergeht. Das Aroma ist eine Mischung aus nussigen, erdigen und dieselartigen Noten mit einem Hauch von Schokolade."
  },
  "killer-new-haven": {
    "description": "Killer New Haven ist eine seltene, reine Indica-Sorte, die als 'Heirloom' (Erbstück) aus der Region Connecticut, USA, stammt. Sie ist einer der Elternteile der potenten Sorte Herijuana. Ihre Wirkung ist stark sedierend und körperlich entspannend, typisch für eine klassische Indica. Das Aroma ist tief erdig, scharf und würzig, oft mit Noten, die an traditionelles Haschisch erinnern."
  },
  "lilly": {
    "description": "Lilly ist eine reine Sativa-Sorte, die für ihr extrem süßes und fruchtiges Aromaprofil bekannt ist, das an Ananas und andere tropische Früchte erinnert. Sie ist eine Kreuzung aus den afrikanischen Sativas Queen Mother und Congo. Die Wirkung ist schnell einsetzend, stark zerebral, euphorisch und energetisierend, was sie zu einer ausgezeichneten Wahl für den Tagesgebrauch und kreative Aktivitäten macht."
  },
  "lucifer-og": {
    "description": "Lucifer OG ist eine potente, indica-dominante Hybridsorte, die für ihre starke und langanhaltende Wirkung bekannt ist. Sie kombiniert die Genetik von Hell's OG mit einer stabilisierten 'In-Bred Line' (IBL) von SFV OG. Die Wirkung ist tief entspannend und sedierend, ideal für erfahrene Konsumenten zur Linderung von Schmerzen und Schlaflosigkeit. Das Aroma ist ein klassisches OG-Profil mit dominanten Noten von Zitrone, Kiefer und Diesel."
  },
  "marleys-collie": {
    "description": "Marley's Collie ist eine sativa-dominante Hybridsorte, die nach dem berühmten Reggae-Musiker Bob Marley benannt ist. Sie kombiniert eine jamaikanische Landrasse mit einer afghanischen Maple Leaf Indica, was zu einer erhebenden und gleichzeitig entspannenden Wirkung führt. Das High ist typischerweise zerebral, kreativ und fröhlich, aber durch den Indica-Einfluss geerdet. Das Aroma ist süß und tropisch mit würzigen und holzigen Untertönen."
  },
  "matanuska-valley-sativa": {
    "description": "Matanuska Valley Sativa ist eine legendäre Landrassensorte aus dem Matanuska-Tal in Alaska, die als ursprünglicher Elternteil von Alaskan Thunder Fuck (ATF) gilt. Sie ist bekannt für ihre Anpassungsfähigkeit an kühlere Klimazonen und ihre starke, erhebende Wirkung. Das High ist typischerweise euphorisch, energetisierend und fördert die Kreativität. Das Aroma ist eine komplexe Mischung aus Kiefer, Zitrone, Menthol und Skunk."
  },
  "nepalese-sativa": {
    "description": "Nepalese Sativa ist eine reine Landrassensorte aus den Bergregionen Nepals. Sie ist bekannt für ihre hohe Harzproduktion, die traditionell zur Herstellung von hochwertigem Charas (Haschisch) genutzt wird. Die Wirkung ist klar, zerebral und erhebend, oft als meditativ und kreativitätsfördernd beschrieben. Das Aroma ist typischerweise süß und kräuterartig mit würzigen und erdigen Noten. Sie ist eine der vermuteten Vorfahren der legendären Chemdawg-Linie."
  }
};
