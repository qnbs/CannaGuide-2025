...
>(({ className, ...props }, ref) => (
    <DialogPrimitive.Overlay
        ref={ref}
        className={cn(
            'fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-sm data-[state=open]:animate-fade-in',
            className,
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <DialogPrimitive.Title
        ref={ref}
        className={cn('text-xl font-bold font-display text-primary-300', className)}
        {...props}
    />
))
