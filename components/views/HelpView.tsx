...
const TabContent: React.FC<{ activeTab: HelpTab, onToggle: (id: string, group: string) => void, openAccordions: Set<string> }> = ({ activeTab, onToggle, openAccordions }) => {
    const { t } = useTranslations();
    
    const faqItems = useMemo(() => Array.from({ length: 7 }).map((_, i) => ({ id: `q${i + 1}`, q: t(`helpView.sections.faq.items.q${i + 1}.q`), a: t(`helpView.sections.faq.items.q${i + 1}.a`) })), [t]);
...
export const HelpView: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            <TabContent activeTab={activeTab} onToggle={handleToggle} openAccordions={openAccordions} />
        </div>
    );
};