...
const StrainsSettingsPanel: React.FC<{ settings: AppSettings; setSetting: (path: string, value: any) => void }> = ({ settings, setSetting }) => {
    ...
    return (
        <SettingsSection title={t('settingsView.strains.title')}>
            ...
            <SelectRow label={t('settingsView.strains.defaultViewMode')} value={settings.strainsViewSettings.defaultViewMode} onChange={e => setSetting('strainsViewSettings.defaultViewMode', e.target.value)}>{Object.keys(t('strainsView.strains.viewModes')).map(k => <option key={k} value={k}>{t(`strainsView.strains.viewModes.${k}`)}</option>)}</SelectRow>
