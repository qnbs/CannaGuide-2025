  ...
  return (
    <Card className='!p-0 overflow-hidden'>
      ...
      {isOpen && (
        <div
          id={sectionId}
          className='px-4 pb-4 border-t border-slate-700/50 animate-fade-in'
        >
...
const SettingRow: React.FC<{
  ...
  <div
    className={`flex flex-col sm:flex-row justify-between sm:items-center gap-2 py-3 first:pt-0 last:pb-0 ${className}`}
  >
  ...
  return (
    <div className='p-3 bg-slate-800/50 rounded-lg'>
      <h4 className='text-md font-semibold text-slate-300 mb-2'>
        {t('settingsView.data.storageUsage')}
      </h4>
      {isLoading ? (
...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const handleImportData = (event: React.ChangeEvent<HTMLInputElement>) => {
    console.log(
      'Import functionality would need a Redux Persist setup to work correctly.'
    )
    dispatch(
      addNotification({
        message: 'Import currently not supported in Redux version.',
        type: 'info',
      })
    )
  }
  ...
  return (
    <div className='space-y-6 animate-fade-in'>
      ...
      <SettingsSection
        title={t('settingsView.categories.general')}
      ...
      >
        <SettingRow label={t('settingsView.general.theme')}>
          <Select
            ...
            options={Object.keys(
              t('settingsView.general.themes', { returnObjects: true })
            ).map((key) => ({
              value: key,
              label: t(`settingsView.general.themes.${key}`),
            }))}
          />
        </SettingRow>
        <SettingRow label={t('settingsView.general.defaultView')}>
          <Select
            ...
            onChange={(e) =>
              handleSetSetting('defaultView', e.target.value as View)
            }
            options={Object.values(View).map((v) => ({
              value: v,
              label: t(`nav.${(v as string).toLowerCase()}`),
            }))}
          />
        </SettingRow>
        <SettingRow
          label={t('settingsView.general.installApp')}
          description={t('settingsView.general.installAppDesc')}
        >
      ...
      <SettingsSection
        title={t('settingsView.categories.plants')}
      ...
      >
        ...
        >
          <Select
            ...
            onChange={(e) =>
              dispatch(setSimulationProfile(e.target.value as any))
            }
            options={Object.keys(
              t('settingsView.plants.simulationProfile.profiles', {
                returnObjects: true,
              })
            ).map((key) => ({
              value: key,
              label: t(
                `settingsView.plants.simulationProfile.profiles.${key}`
              ),
            }))}
          />
        </SettingRow>
        ...
        >
          <input
            ...
            step='0.1'
            value={
              settings.simulationSettings.customDifficultyModifiers
                .pestPressure
            }
            onChange={(e) =>
              handleSetSetting(
                'simulationSettings.customDifficultyModifiers.pestPressure',
          ...
          <span className='font-mono text-sm'>
            {settings.simulationSettings.customDifficultyModifiers.pestPressure.toFixed(
              1
            )}
            x
          </span>
        ...
        >
          <input
            ...
            onChange={(e) =>
              handleSetSetting(
                'simulationSettings.customDifficultyModifiers.nutrientSensitivity',
                parseFloat(e.target.value)
              )
            }
            className='w-32 accent-primary-500'
          />
          <span className='font-mono text-sm'>
            {settings.simulationSettings.customDifficultyModifiers.nutrientSensitivity.toFixed(
              1
            )}
            x
          </span>
        ...
        >
          <input
            ...
            onChange={(e) =>
              handleSetSetting(
                'simulationSettings.customDifficultyModifiers.environmentalStability',
                parseFloat(e.target.value)
              )
            }
            className='w-32 accent-primary-500'
          />
          <span className='font-mono text-sm'>
            {settings.simulationSettings.customDifficultyModifiers.environmentalStability.toFixed(
              1
            )}
            x
          </span>
      ...
      <SettingsSection
        title={t('settingsView.categories.data')}
        icon={<PhosphorIcons.Archive />}
      >
        <StorageBreakdown />
        <SettingRow
          label={t('settingsView.data.exportAll')}
          description={
            t('settingsView.data.lastBackup') +
            ': ' +
            (settings.lastBackupTimestamp
              ? new Date(settings.lastBackupTimestamp).toLocaleString()
              : t('settingsView.data.noBackup'))
          }
        >
        ...
        <SettingRow
          label={t('settingsView.data.clearArchives')}
          description={t('settingsView.data.clearArchivesDesc')}
        >
          <Button onClick={handleClearArchives} variant='danger'>
            {t('settingsView.data.clearArchives')}
          </Button>
        </SettingRow>
        <SettingRow label={t('settingsView.data.replayOnboarding')}>
          <Button onClick={handleReplayOnboarding} variant='secondary'>
            {t('settingsView.data.replayOnboarding')}
          </Button>
        </SettingRow>
        <SettingRow label={t('settingsView.data.resetPlants')}>
          <Button onClick={handleResetPlants} variant='danger'>
            {t('settingsView.data.resetPlants')}
          </Button>
        </SettingRow>
        <SettingRow label={t('settingsView.data.resetAll')}>
          <Button onClick={handleResetAll} variant='danger'>
