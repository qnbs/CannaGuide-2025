...
import { EditStrainTipModal } from './strains/EditStrainTipModal';
...
const TipItem: React.FC<{ tip: SavedStrainTip, onEdit: (tip: SavedStrainTip) => void, onDelete: (id: string) => void, onSelect: (id: string) => void, isSelected: boolean }> = ({ tip, onEdit, onDelete, onSelect, isSelected }) => {
    ...
    return (
        <div className="flex gap-3 items-start overflow-hidden">
            <input type="checkbox" checked={isSelected} onChange={() => onSelect(tip.id)} className="mt-1 h-4 w-4 rounded border-slate-500 bg-transparent text-primary-500 focus:ring-primary-500 flex-shrink-0" />
            <div className="flex-1">
                ...
                <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        <Button size="sm" variant="secondary" onClick={() => onEdit(tip)} aria-label={t('common.edit')}><PhosphorIcons.PencilSimple className="w-4 h-4" /></Button>
                        <Button size="sm" variant="danger" onClick={() => onDelete(tip.id)} aria-label={t('common.delete')}><PhosphorIcons.TrashSimple className="w-4 h-4" /></Button>
                    </div>
                </div>
...
const StrainTipsView: React.FC<StrainTipsViewProps> = ({ savedTips, deleteTip, updateTip, allStrains }) => {
    const { t } = useTranslation();
             ...
             {savedTips.length === 0 ? (
                ...
                <div className="space-y-3">
                    <div className="px-3 flex items-center gap-3">
                        ...
                        <label className="text-sm text-slate-400">{t('strainsView.selectedCount_other', { count: selectedIds.size })}</label>
                    </div>
                    {sortMode === 'grouped' ? (
                             ...
                             return (
                                <details key={strainName} open={true} className="group ring-1 ring-inset ring-white/20 rounded-lg overflow-hidden">
                                    ...
                                    <div className="p-3 space-y-3 bg-slate-800/40">
                                        {tips.map(tip => (<Card key={tip.id} className="bg-slate-800/50 p-3 overflow-hidden"><TipItem tip={tip} onEdit={setEditingTip} onDelete={deleteTip} onSelect={handleToggleSelection} isSelected={selectedIds.has(tip.id)} /></Card>))}
                                    </div>
                                </details>
                            )
                        })
                    ) : (
                        (sortedAndGrouped as SavedStrainTip[]).map(tip => (<Card key={tip.id} className="bg-slate-800 p-3 ring-1 ring-inset ring-white/20"><TipItem tip={tip} onEdit={setEditingTip} onDelete={deleteTip} onSelect={handleToggleSelection} isSelected={selectedIds.has(tip.id)} /></Card>))
                    )}
                </div>
            )}
        </div>
