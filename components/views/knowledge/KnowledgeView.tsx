            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    {Object.entries(checklist).map(([key, text]) => (
                        <label key={key} htmlFor={`${baseId}-${key}`} className="flex items-center gap-3 cursor-pointer">
                            <input id={`${baseId}-${key}`} name={`${phase}-${key}`} type="checkbox" checked={progress.includes(key)} onChange={() => onToggle(key)} className="h-4 w-4 rounded border-slate-500 bg-transparent text-primary-500 focus:ring-primary-500"/>
