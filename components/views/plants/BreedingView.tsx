...
const SeedCard: React.FC<{
    ...
    return (
        <button
            onClick={onClick}
            className={`w-full min-h-12 text-left p-3 rounded-lg transition-all ring-1 ring-inset ring-white/20 flex items-center gap-3 ${isSelected ? 'bg-primary-900/50 ring-2 ring-primary-500' : 'bg-slate-800 hover:bg-slate-700/50'}`}
        >
            {TypeInfo && (
...
const ParentSlot: React.FC<{
    ...
    const safeThc =
        typeof parentStrain?.thc === 'number' && Number.isFinite(parentStrain.thc)
            ? parentStrain.thc
            : 0
    const safeCbd =
        typeof parentStrain?.cbd === 'number' && Number.isFinite(parentStrain.cbd)
            ? parentStrain.cbd
            : 0
            ...
            {seed && parentStrain && TypeInfo ? (
                <>
                    ...
                    <div className="text-xs text-slate-400 mt-1">
                        <p>
                            THC: {safeThc.toFixed(1)}% | CBD: {safeCbd.toFixed(1)}%
                        </p>
                        <p>
                            {t('common.quality')}: {(seed.quality * 100).toFixed(0)}%
                        </p>
            ...
            {result && parentA && parentB && (
                <Card className="animate-fade-in bg-slate-800/50">
                    <div className="flex justify-between items-center">
                        ...
                        <Button variant="secondary" onClick={handleReset} className="min-h-11">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        <div className="space-y-4">
                            ...
                            <TraitComparison
                                label="THC"
                                valA={`${parentA.thc.toFixed(1)}%`}
                                valB={`${parentB.thc.toFixed(1)}%`}
                                valChild={`~${result.thc.toFixed(1)}%`}
                                icon={<PhosphorIcons.Lightning weight="fill" />}
                            />
                            <TraitComparison
                                label={t('knowledgeView.breeding.flowering')}
                                valA={`${parentA.floweringTime} w`}
                                valB={`${parentB.floweringTime} w`}
                                valChild={`~${result.floweringTime.toFixed(0)} w`}
