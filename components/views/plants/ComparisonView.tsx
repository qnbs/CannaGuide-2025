...
const ComparisonChart: React.FC<{
    ...
    labelA: string
    labelB: string
}> = memo(({ historyA, historyB, labelA, labelB }) => {
        ...
        return (
            <div className="w-full">
                <h4 className="font-bold text-center mb-2">
                    {t('plantsView.detailedView.height')} (cm) vs. {t('plantsView.plantCard.day')}
                </h4>
                <svg
                    viewBox={`0 0 ${COMP_WIDTH} ${COMP_HEIGHT}`}
                    className="w-full"
                    aria-label={t('plantsView.comparison.chartLabel')}
                >
                    <title>{t('plantsView.comparison.chartLabel')}</title>
                    <g
                        className="history-chart-grid"
                        transform={`translate(0, ${COMP_HEIGHT - COMP_PADDING.bottom})`}
                    >
                    ...
                    <g
                        className="history-chart-grid"
                        transform={`translate(${COMP_PADDING.left}, 0)`}
                    >
                ...
                <div className="flex justify-center gap-4 text-xs mt-2">
                    <span className="flex items-center gap-1.5">
                        <div className="w-3 h-1.5 bg-primary-500"></div>
                        {labelA}
                    </span>
                    <span className="flex items-center gap-1.5">
                        <div className="w-3 h-1.5 bg-accent-500"></div>
                        {labelB}
                    </span>
                </div>
            </div>
