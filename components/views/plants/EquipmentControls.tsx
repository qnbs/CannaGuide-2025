...
const InfoDisplay: React.FC<{ label: string; value: string; tooltip: string }> = ({
    label,
    value,
    tooltip,
}) => (
    ...
    return (
        <Card className="h-full">
            <h3 className="text-xl font-bold font-display text-primary-400 mb-4">
                {t('plantsView.detailedView.controls.title')}
            </h3>
            <div className="space-y-3">
                ...
                <div className="p-3 bg-slate-800/50 rounded-lg space-y-4">
                        ...
                        <h4 className="font-bold text-slate-100 flex items-center gap-2">
                            {light.isOn ? (
                                <PhosphorIcons.LightbulbFilament
                                    weight="fill"
                                    className="w-5 h-5 text-yellow-300 drop-shadow-[0_0_5px_rgba(252,211,77,0.8)]"
                                />
                            ) : (
                    ...
                    >
                        <RangeSlider
                            ...
                            value={light.wattage}
                            onChange={(val) =>
                                dispatch(setLightWattage({ plantId: plant.id, wattage: val }))
                            }
                            unit="W"
                            singleValue
                        />
                ...
                <div className="p-3 bg-slate-800/50 rounded-lg space-y-4">
                    <div className="flex justify-between items-center">
                        <h4 className="font-bold text-slate-100 flex items-center gap-2">
                            <PhosphorIcons.Fan
                                className={exhaustFan.isOn ? 'animate-spin' : ''}
                                style={{
                                    animationDuration: fanAnimationDurations[exhaustFan.power],
                                }}
                            />
                    ...
                    <div className="flex justify-between items-center border-t border-slate-700/50 pt-4 mt-4">
                        <h4 className="font-bold text-slate-100 flex items-center gap-2">
                            <PhosphorIcons.Fan
                                className={circulationFan.isOn ? 'animate-spin' : ''}
                            ...
                            <span className="group relative">
                                <PhosphorIcons.Question className="w-4 h-4 text-slate-400" />
                                <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-900 text-slate-200 text-xs rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                    {t('plantsView.detailedView.controls.circulationFanTooltip')}
                                </span>
                            </span>
                        </h4>
                        <Switch
...
EquipmentControls.displayName = 'EquipmentControls'
