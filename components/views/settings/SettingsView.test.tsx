...
describe('SettingsView navigation smoke', () => {
    beforeEach(() => {
        vi.clearAllMocks()
        // usePwaInstall calls window.matchMedia which isn't available in jsdom
    ...
    const tabCases = [
        {
            label: 'settingsView.categories.general',
            expectedHeading: /settingsView.categories.general/i,
        },
    ...
    it.each(tabCases)(
        'opens tab %s without crash',
        async ({ label, expected, expectedHeading }) => {
            render(<SettingsView />)
