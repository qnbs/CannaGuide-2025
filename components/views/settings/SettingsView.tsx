...
interface SectionProps {
    ...
    handleSetSetting: (path: string, value: unknown) => void;
    t: TFunction;
}

const LanguageSelectionCard: React.FC<{ handleLanguageSelect: (lang: Language) => void }> = memo(
    ...
    >
        <SettingRow label={t('settingsView.general.theme')}>
            <Select
                ...
                options={Object.keys(t('settingsView.general.themes', { returnObjects: true })).map(
                    (key) => ({ value: key, label: t(`settingsView.general.themes.${key}`) })
                )}
            />
        </SettingRow>
...
const PlantsAndSimulationSettings: React.FC<SectionProps> = memo(
    ({ settings, dispatch, handleSetSetting, t }) => (
        <SettingsSection title={t('settingsView.categories.plants')} icon={<PhosphorIcons.Plant />}>
            ...
            >
                ...
                <span className='font-mono text-sm'>
                    {settings.simulationSettings.customDifficultyModifiers.pestPressure.toFixed(1)}x
                </span>
            </SettingRow>
            <SettingRow
            ...
            >
                <input
                    ...
                    step='0.1'
                    value={
                        settings.simulationSettings.customDifficultyModifiers.environmentalStability
                    }
                    onChange={(e) =>
                        handleSetSetting(
