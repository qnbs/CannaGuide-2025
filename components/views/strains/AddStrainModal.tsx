    ...
    <div className="rounded-2xl border border-white/10 bg-slate-900/45 p-3">
        ...
        <div className="mt-2 flex min-h-8 flex-wrap gap-2">
            {values.length > 0 ? values.map((value) => (
                <span key={value} className="rounded-full border border-white/10 bg-white/6 px-2.5 py-1 text-xs text-slate-200">
                    {value}
                </span>
            )) : (
...
export const AddStrainModal: React.FC<AddStrainModalProps> = ({ isOpen, onAddStrain, onUpdateStrain, strainToEdit, ...props }) => {
    ...
    const parsedAromas = useMemo(
        () => values.aromasString.split(/\s*,\s*/).map((item) => item.trim()).filter(Boolean),
        [values.aromasString],
    );

    const parsedTerpenes = useMemo(
        () => values.terpenesString.split(/\s*,\s*/).map((item) => item.trim()).filter(Boolean),
        [values.terpenesString],
    );

    const typeOptions = useMemo(
        () => [
            { value: StrainType.Sativa, label: t('strainsView.sativa'), icon: <PhosphorIcons.ArrowUp className="h-4 w-4" /> },
            { value: StrainType.Hybrid, label: t('strainsView.hybrid'), icon: <PhosphorIcons.Sparkle className="h-4 w-4" /> },
            { value: StrainType.Indica, label: t('strainsView.indica'), icon: <PhosphorIcons.ArrowDown className="h-4 w-4" /> },
        ],
        [t],
    );
    ...
    const footer = (
        <>
            <Button type="button" variant="secondary" onClick={props.onClose} className="min-h-11 w-full sm:w-auto">{t('common.cancel')}</Button>
            <Button type="submit" form="add-strain-form" glow={true} className="min-h-11 w-full sm:w-auto">{t('common.save')}</Button>
        </>
    );
        ...
        >
            <form onSubmit={handleSubmit} id="add-strain-form" className="flex h-full flex-col">
                <div className="space-y-5 pb-4">
                    <Card className="overflow-hidden border-white/10 bg-[linear-gradient(135deg,rgba(14,116,144,0.18),rgba(15,23,42,0.92))] !p-0">
                        <div className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1.3fr)_minmax(18rem,0.9fr)] lg:items-start">
                            ...
                            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                                ...
                                <TokenPreview title={t('strainsView.addStrainModal.terpenesPreview')} values={parsedTerpenes.slice(0, 4)} />
                            </div>
                        </div>
                    </Card>

                    <FormSection title={t('strainsView.addStrainModal.generalInfo')} defaultOpen={true} icon={<PhosphorIcons.Question className="h-5 w-5" />}>
                        <div className="sm:col-span-2">
                            <Input label={`${t('strainsView.addStrainModal.strainName')} *`} value={values.name} onChange={(e) => handleChange('name', e.target.value)} placeholder={t('strainsView.addStrainModal.strainNamePlaceholder')} required />
                            <ErrorText message={errors.name} />
                        </div>
                        <div className="sm:col-span-2">
                            <p className="mb-2 text-sm font-semibold text-slate-300">{t('strainsView.addStrainModal.typeLegend')}</p>
                            <SegmentedControl
                        ...
                        <Input label={t('common.genetics')} value={values.genetics} onChange={(e) => handleChange('genetics', e.target.value)} placeholder={t('strainsView.addStrainModal.geneticsPlaceholder')} />
                        <div className="sm:col-span-2">
                            <p className="mb-2 text-sm font-semibold text-slate-300">{t('strainsView.addStrainModal.floweringTypeLegend')}</p>
                            <SegmentedControl
                                aria-label={t('strainsView.addStrainModal.floweringTypeLegend')}
                                value={[values.isAutoflower ? 'Autoflower' : 'Photoperiod']}
                                onToggle={(value) => handleChange('isAutoflower', value === 'Autoflower')}
                                options={[
                                    { value: 'Photoperiod', label: t('strainsView.addStrainModal.photoperiod'), icon: <PhosphorIcons.Sun className="h-4 w-4" /> },
                                    { value: 'Autoflower', label: t('strainsView.addStrainModal.autoflower'), icon: <PhosphorIcons.Lightning className="h-4 w-4" /> },
                                ]}
                                buttonClassName="min-h-11"
                            />
                        </div>
                    ...
                    <FormSection title={t('strainsView.addStrainModal.cannabinoids')} icon={<PhosphorIcons.Drop className="h-5 w-5" />}>
                        <div>
                            <Input label={`${t('strainsView.addStrainModal.thcPercent')} *`} type="number" step="0.1" value={values.thc} onChange={(e) => handleChange('thc', Number(e.target.value))} required />
                            <ErrorText message={errors.thc} />
                        </div>
                        <Input label={t('strainsView.addStrainModal.cbdPercent')} type="number" step="0.1" value={values.cbd} onChange={(e) => handleChange('cbd', Number(e.target.value))} />
                        <div>
                    ...
                    <FormSection title={t('strainsView.addStrainModal.growData')} icon={<PhosphorIcons.ChartLineUp className="h-5 w-5" />}>
                        <div>
                            <Input label={`${t('strainsView.addStrainModal.floweringTimeWeeks')} *`} type="number" step="0.5" value={values.floweringTime} onChange={(e) => handleChange('floweringTime', Number(e.target.value))} required />
                        ...
                        <div className="sm:col-span-2 grid gap-4 lg:grid-cols-3">
                            <div>
                                <p className="mb-2 text-sm font-semibold text-slate-300">{t('strainsView.table.difficulty')}</p>
                                <SegmentedControl
                                    aria-label={t('strainsView.table.difficulty')}
                                    value={[values.difficulty]}
                                    onToggle={(value) => handleChange('difficulty', value as DifficultyLevel)}
                                    options={[
                                        { value: 'Easy', label: t('strainsView.difficulty.easy') },
                                        { value: 'Medium', label: t('strainsView.difficulty.medium') },
                            ...
                            <div>
                                <p className="mb-2 text-sm font-semibold text-slate-300">{t('strainsView.addStrainModal.yield')}</p>
                                <SegmentedControl
                                    aria-label={t('strainsView.addStrainModal.yield')}
                                    value={[values.yield]}
                                    onToggle={(value) => handleChange('yield', value as YieldLevel)}
                                    options={[
                                        { value: 'Low', label: t('strainsView.addStrainModal.yields.low') },
                                        { value: 'Medium', label: t('strainsView.addStrainModal.yields.medium') },
                                        { value: 'High', label: t('strainsView.addStrainModal.yields.high') },
                                    ]}
                                    className="grid grid-cols-3"
                                />
                            </div>
                            <div>
                                <p className="mb-2 text-sm font-semibold text-slate-300">{t('strainsView.addStrainModal.height')}</p>
                                <SegmentedControl
                                    aria-label={t('strainsView.addStrainModal.height')}
                                    value={[values.height]}
                                    onToggle={(value) => handleChange('height', value as HeightLevel)}
                                    options={[
                                        { value: 'Short', label: t('strainsView.addStrainModal.heights.short') },
                                        { value: 'Medium', label: t('strainsView.addStrainModal.heights.medium') },
                                        { value: 'Tall', label: t('strainsView.addStrainModal.heights.tall') },
                                    ]}
                                    className="grid grid-cols-3"
                                />
                            </div>
                    ...
                    <FormSection title={t('strainsView.addStrainModal.profile')} icon={<PhosphorIcons.Sparkle className="h-5 w-5" />}>
                        <div className="sm:col-span-2">
                            <Input as="textarea" rows={3} label={t('common.description')} value={values.description} onChange={(e) => handleChange('description', e.target.value)} />
                        </div>
