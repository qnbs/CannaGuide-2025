...
import type { Strain, StructuredGrowTips } from '@/types'
...
const StructuredTipDisplay: React.FC<{
    tips: StructuredGrowTips
    onSave: () => void
    isSaved: boolean
    strainId: string
}> = ({ tips, onSave, isSaved, strainId }) => {
...
export const StrainAiTips: React.FC<StrainAiTipsProps> = ({ strain }) => {
    ...
    const [getStrainTips, { data: tip, isLoading: isTextLoading, error: textError }] =
        useGetStrainTipsMutation({
            fixedCacheKey: `strain-tip-text-${strain.id}`,
        })
    const [
        generateStrainImage,
        { data: generatedImageData, isLoading: isImageLoading, error: imageError },
    ] = useGenerateStrainImageMutation({
        fixedCacheKey: `strain-tip-image-${strain.id}`,
    })
    const imageUrl = normalizeImageDataUrl(generatedImageData)

    const isLoading = isTextLoading || isImageLoading
    const error = textError || imageError

    const [loadingMessage, setLoadingMessage] = useState('')
    const [savedTipStrainId, setSavedTipStrainId] = useState<string | null>(null)
    const isTipSaved = savedTipStrainId === strain.id
    ...
    const hasGeneratedOnce = !!tip || !!imageUrl

    useEffect(() => {
        if (isLoading) {
            const experienceText = t(
                `strainsView.tips.form.experienceOptions.${tipRequest.experienceLevel}`,
            )
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <Select
                        ...
                        options={Object.keys(
                            t('strainsView.tips.form.focusOptions', { returnObjects: true }),
                        ).map((k) => ({
                            value: k,
                    ...
                    <Select
                        ...
                        options={Object.keys(
                            t('strainsView.tips.form.stageOptions', { returnObjects: true }),
                        ).map((k) => ({
                            value: k,
                            label: t(`strainsView.tips.form.stageOptions.${k}`),
                        }))}
                    />
                    <Select
                ...
                {isImageLoading ? (
                    ...
                    imageUrl && (
                        <div className="mt-4 animate-fade-in">
                            <img
                                src={imageUrl}
                                alt={t('ai.prompts.strainImage', { strainName: strain.name })}
