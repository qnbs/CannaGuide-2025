    ...
    return (
        <>
            {selectedImage && (
                ...
                >
                    <img src={selectedImage} alt="Full screen strain" className="max-w-full max-h-full object-contain rounded-lg shadow-2xl" loading="eager" decoding="async" onClick={e => e.stopPropagation()} />
                </div>
            )}
                ...
                {images.map((image, index) => (
                    <Card key={index} className="p-0 overflow-hidden cursor-pointer group" onClick={() => setSelectedImage(image.url)}>
                        <div className="relative">
                            <img src={image.url} alt={image.title} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" decoding="async" />
