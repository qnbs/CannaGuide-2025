...
| Measure            | Implementation                                                          |
| ------------------ | ----------------------------------------------------------------------- |
| SLSA Provenance    | Level 3 via `slsa-github-generator` (isolated runner, non-falsifiable)  |
...
| Release Pipeline   | 3-job isolation: build -> provenance -> release                         |
| Verification (L1)  | `gh attestation verify cannaguide-*.tar.gz --repo qnbs/CannaGuide-2025` |
| Verification (L3)  | `slsa-verifier verify-artifact --provenance-path *.intoto.jsonl`        |
| Release Assets     | Tarball + SBOM + L3 provenance `.intoto.jsonl`                          |
...
pnpm test                 # Vitest unit/integration (2063 tests)
