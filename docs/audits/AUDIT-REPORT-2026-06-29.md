...
| Severity | Open (this audit) | Resolved in this session |
| -------- | ----------------- | ------------------------ |
| P0       | 0 (after fixes)   | 3                        |
| P1       | 6                 | 2                        |
| P2       | 8                 | 0                        |
...
**CI health at audit time:** `typecheck` ✅ | `pnpm audit` 12 vulns (0 critical prod blockers) | Gitleaks not installed locally (runs in CI)
...
| ----- | ----- |
| **Severity** | P1 |
| **Files** | `SettingsView.tsx` (2570), `BreedingLab.tsx` (1199), `GenealogyView.tsx` (1075), `StrainDetailView.tsx` (1057), `dbService.ts` (928), `GrowRoom3D.tsx` (849), `knowledgeGraphService.ts` (777) |
...
2,688+ tests across 234 files; Playwright E2E (Chromium required, Firefox/WebKit advisory); Stryker mutation testing configured.
...
### U-03 — SettingsView god-file impacts maintainability (P1)

See A-01. Settings hosts AI security, cost tracking, sync, IoT — prime split candidate.
...
### Short-term (P1) — Next sprints

1. Coverage sprint: critical paths → 80%
2. Split `SettingsView.tsx` and top 3 strain views
3. Strain data versioning ADR + enrichment cadence
4. Update stale documentation
5. `undici` override bump for jsdom chain
6. Local AI battery UX warnings
...
*Report generated as part of Ultimate Full-Scale Audit v2.0. Code fixes committed on branch `cursor/full-scale-audit-2026-ec29`.*
