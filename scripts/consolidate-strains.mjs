...
function extractStrainBlocks(content) {
  ...
  while (idx < content.length) {
    ...
    if (nameMatch) {
      ...
      console.warn(`  [WARN] Could not extract name from block at position ${start}`);
    }
...
for (const [letter, items] of Object.entries(buckets).sort()) {
  console.log(`  ${letter.toUpperCase()}: ${items.length} strains (${items.map(i => i.name).join(', ')})`);
}
console.log();
...
for (const [letter, items] of Object.entries(buckets)) {
  ...
  for (const item of items) {
    if (item.id && existingIds.has(item.id)) {
      console.log(`  [SKIP] Skipping duplicate: "${item.name}" (${item.id}) already in ${targetFile}`);
      skipCount++;
      continue;
    }
    newBlocks.push(item);
...
console.log(`   [DIR] ${finalFiles.length} strain data files (was ${alphaFiles.length + tempFiles.length})`);
