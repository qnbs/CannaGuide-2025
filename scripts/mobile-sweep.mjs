...
const BASE_URL =
  process.env.DEPLOY_BASE_URL || "https://qnbs.github.io/CannaGuide-2025/";
...
const screens = [
  {
    id: "plants",
    run: async (p) => {
      await clickBottomNavByView(p, "plants");
    },
  },
  {
    id: "strains-all",
    run: async (p) => {
      await clickBottomNavByView(p, "strains");
      await clickSubNavIndex(p, 0);
    },
  },
  {
    id: "strains-my",
    run: async (p) => {
      await clickBottomNavByView(p, "strains");
      await clickSubNavIndex(p, 1);
    },
  },
  {
    id: "strains-favorites",
    run: async (p) => {
      await clickBottomNavByView(p, "strains");
      await clickSubNavIndex(p, 2);
    },
  },
  {
    id: "strains-genealogy",
    run: async (p) => {
      await clickBottomNavByView(p, "strains");
      await clickSubNavIndex(p, 3);
    },
  },
  {
    id: "strains-exports",
  ...
  },
  {
    ...
    run: async (p) => {
      await clickBottomNavByView(p, "strains");
      await clickSubNavIndex(p, 5);
    },
  },
  {
    id: "equipment-config",
    run: async (p) => {
      await clickBottomNavByView(p, "equipment");
      await clickSubNavIndex(p, 0);
    },
  },
  {
  ...
  },
  {
    id: "knowledge-mentor",
    run: async (p) => {
      await clickBottomNavByView(p, "knowledge");
      await clickSubNavIndex(p, 0);
    },
  },
  {
  ...
  return page.evaluate(() => {
    ...
    const nav =
      navCandidates.find((candidate) =>
        candidate.querySelector("[data-view-id]"),
      ) || null;
    ...
    for (const el of nodes) {
      ...
      const centerVisibleInMain =
        centerY >= mainRect.top && centerY <= mainRect.bottom;
      const centerVisibleInViewport =
...
async function run() {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ baseUrl: BASE_URL, report }, null, 2),
  );
  await browser.close();

  const total = report.reduce(
    (sum, r) => sum + r.topIssues.length + r.bottomIssues.length,
    0,
  );
