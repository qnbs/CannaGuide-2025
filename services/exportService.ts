...
const drawPdfLayout = (doc: jsPDF, t: TFunction, title: string) => {
...
const exportData = <T extends { [key: string]: any }>(
  ...
  config: {
    title: string;
    toSerializable: (item: T, t: TFunction) => any;
    xmlRoot: string;
    xmlItem: string;
    txtFormatter: (item: T, t: TFunction) => string;
    pdfHeaders: string[];
    pdfRows: (item: T, t: TFunction) => string[];
  }
) => {
    const serializableData = data.map(item => config.toSerializable(item, t));
    ...
    switch (format) {
        ...
        case 'txt':
            ...
            data.forEach(item => {
                txtString += `----------------------------------------\n`;
                txtString += config.txtFormatter(item, t);
                txtString += `\n`;
            });
        ...
        case 'pdf':
            ...
            (doc as any).autoTable({
                ...
                body: data.map(item => config.pdfRows(item, t)),
                startY: 30,
                didDrawPage: () => drawPdfLayout(doc, t, config.title),
                styles: { fontSize: 8 },
...
const strainToSerializableObject = (strain: Strain, t: TFunction) => ({
...
const exportStrains = (strains: Strain[], format: ExportFormat, filename: string, t: TFunction) => {
    exportData(strains, format, filename, t, {
        title: t('strainsView.exportModal.title'),
        toSerializable: strainToSerializableObject,
        xmlRoot: 'strains', xmlItem: 'strain',
        txtFormatter: (s, tFunc) => {
            const data = strainToSerializableObject(s, tFunc);
            let str = `[ ${data[tFunc('strainsView.csvHeaders.name')]} ]\n`;
            for (const [key, value] of Object.entries(data)) {
                if (value && key !== tFunc('strainsView.csvHeaders.name')) {
                    str += `${key}: ${value}\n`;
                }
            }
            return str;
        ...
        pdfRows: (s, tFunc) => [s.name, s.type, `${s.thc}%`, `${s.cbd}%`, `${s.floweringTimeRange || s.floweringTime} ${tFunc('common.units.weeks')}`, tFunc(`strainsView.difficulty.${s.agronomic.difficulty.toLowerCase()}`), s.agronomic.yieldDetails?.indoor || 'N/A', s.agronomic.heightDetails?.indoor || 'N/A'],
    });
};

// --- Setup Export ---

const setupToSerializableObject = (setup: SavedSetup, t: TFunction) => ({
    name: setup.name, createdAt: new Date(setup.createdAt).toISOString(), totalCost: setup.totalCost, sourceDetails: setup.sourceDetails,
    components: (Object.keys(setup.recommendation) as RecommendationCategory[]).map(key => ({
        category: t(`equipmentView.configurator.categories.${key}`), product: setup.recommendation[key].name, price: setup.recommendation[key].price,
        rationale: setup.recommendation[key].rationale, watts: setup.recommendation[key].watts
    }))
});

const exportSetup = (setup: SavedSetup, format: ExportFormat, t: TFunction) => {
    const filename = setup.name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    const serializableSetup = setupToSerializableObject(setup, t);
    ...
    switch (format) {
        ...
        case 'pdf':
            ...
            drawPdfLayout(doc, t, reportTitle);
            ...
            (doc as any).autoTable({
                ...
                startY: 50,
                headStyles: { fillColor: [37, 99, 235] }, // primary-600
                didDrawPage: () => drawPdfLayout(doc, t, reportTitle),
            });
            doc.save(`${filename}.pdf`);
            break;
    }
...
const exportSetups = (setups: SavedSetup[], format: ExportFormat, filename: string, t: TFunction) => {
    exportData(setups, format, filename, t, {
        ...
        txtFormatter: (s, tFunc) => {
            ...
            str += `Total Cost: ${s.totalCost.toFixed(2)} ${tFunc('common.units.currency_eur')}\n`;
            ...
            (Object.keys(s.recommendation) as RecommendationCategory[]).forEach(key => {
                ...
                str += `- ${tFunc(`equipmentView.configurator.categories.${key}`)}: ${item.name} (${item.price.toFixed(2)} ${tFunc('common.units.currency_eur')})\n`;
            });
            return str;
        },
        ...
        pdfRows: (s, tFunc) => [s.name, new Date(s.createdAt).toLocaleDateString(), s.totalCost.toFixed(2), s.sourceDetails.area, s.sourceDetails.growStyle, s.sourceDetails.budget],
    });
};

// --- AI Response Exports ---

const exportStrainTips = (tips: SavedStrainTip[], format: ExportFormat, filename: string, t: TFunction) => {
    exportData(tips, format, filename, t, {
        ...
        txtFormatter: (item, tFunc) => `Strain: ${item.strainName}\nDate: ${new Date(item.createdAt).toLocaleString()}\nTitle: ${item.title}\n---\n${cleanHtml(item.content)}\n`,
...
const exportMentorArchive = (responses: ArchivedMentorResponse[], format: ExportFormat, filename: string, t: TFunction) => {
    exportData(responses, format, filename, t, {
        ...
        txtFormatter: (item, tFunc) => `Query: ${item.query}\nDate: ${new Date(item.createdAt).toLocaleString()}\nTitle: ${item.title}\n---\n${cleanHtml(item.content)}\n`,
...
const exportAdvisorArchive = (responses: (ArchivedAdvisorResponse & {plantName: string})[], format: ExportFormat, filename: string, t: TFunction) => {
    exportData(responses, format, filename, t, {
        title: t('plantsView.aiAdvisor.archiveTitle'),
        toSerializable: (item, tFunc) => ({ Plant: item.plantName, Stage: tFunc(`plantStages.${item.plantStage}`), Created: new Date(item.createdAt).toLocaleString(), Title: item.title, Content: cleanHtml(item.content) }),
        xmlRoot: 'advisor_responses', xmlItem: 'response',
        txtFormatter: (item, tFunc) => `Plant: ${item.plantName}\nStage: ${tFunc(`plantStages.${item.plantStage}`)}\nDate: ${new Date(item.createdAt).toLocaleString()}\nTitle: ${item.title}\n---\n${cleanHtml(item.content)}\n`,
        pdfHeaders: ['Plant', 'Stage', 'Title', 'Date', 'Content'],
        pdfRows: (item, tFunc) => [item.plantName, tFunc(`plantStages.${item.plantStage}`), item.title, new Date(item.createdAt).toLocaleDateString(), cleanHtml(item.content)],
    });
};
