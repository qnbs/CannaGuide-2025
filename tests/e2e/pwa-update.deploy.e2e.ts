...
test('pwa update: service worker registers with GitHub Pages subpath scope', async ({ page, baseURL }) => {
  ...
  const targetRegistration = registrations.find((registration) => {
    ...
    return scopeUrl.pathname === expectedPathPrefix || scopeUrl.pathname.startsWith(expectedPathPrefix)
  })

  expect(targetRegistration).toBeDefined()
  expect(targetRegistration?.hasActiveOrWaiting).toBeTruthy()

  const scriptPathname = targetRegistration?.scriptURL ? new URL(targetRegistration.scriptURL).pathname : ''
